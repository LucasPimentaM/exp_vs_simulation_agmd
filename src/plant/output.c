...
PetscErrorCode ExportToFile(Vec *vector, EntryData *entry_data, char file[])
{
    ...
    GOR = array[12] * dessal_data.membrane_area / GOR;
    ...
    PetscFPrintf(PETSC_COMM_WORLD, fptr, "Permeate flow rate =, %.10f, g/min\n", 60.0 * 1000.0 * array[10] * entry_data->dessal_data.membrane_area);
